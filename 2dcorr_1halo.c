...
main(int argc,char **argv)
{
  ...
  for (i=0;i<np1;i++)  {		    
    ...
    ********************************************/
      ...
      if(rbin<nrbin)
	{
	  ...
	  dz=fabs(z1[i]-z2[j]);
	  //	  if(kbin==4 && rbin==1)
	  //  printf("BOO %d %d %f %f %f\n",kbin,rbin,r,dz,(vz2[j]-vz1[i])*(z2[j]-z1[i])/dz);
  ...
  for (kbin=0;kbin<nbin;kbin++)  {	/* loop over radial bins */
    ...
    */
	...
	fac=0.0 ;
      ...
      diff=jackweight/weightrandom-xi ;	  /* sample xi minus mean */
      errxi+=diff*diff ;
    }
  ...
  for (kbin=0;kbin<nbin;kbin++)  {	/* loop over proj-sep bins */
	...
	printf("%f %f %e %e %d %.0f\n",sigbar[kbin][rbin],pibar[kbin][rbin],xi,errxi,npair2d[kbin][rbin],weightrandom*8./7.);
	/*
