...
main(int argc,char **argv)
{
  ...
  for (i=0;i<np1;i++)  {		    
      ...
      if(rbin<nrbin)
	{
	  ...
	  sigoct[kbin][rbin][ioct]+=weight*r;
	  pioct[kbin][rbin][ioct]+=weight*dz;
	}
	...
	for(ioct=0;ioct<8;++ioct)
	  {
	    ...
	    if (weight0>0.0)  {
	      fac=1./weight0 ;
	      sigoct[kbin][rbin][ioct] *= fac ;
	      pioct[kbin][rbin][ioct] *= fac;
	    }
	    else  {					/* avoid errors in empty bins */
	      fac=0.0 ;
	      sigoct[kbin][rbin][ioct]=(rupp[kbin]+rlow)*0.5;
	      pioct[kbin][rbin][ioct]=(rupp[rbin]+los_low)*0.5;
	    }
	    ...
	    weightrandom=np1*avgweight1*density*vol*7.0/8.0 ; 
	    /* 7/8 is volume of jacknife sample */	
	    ...
	    printf("%f %f %f",xi,sigoct[kbin][rbin][ioct],pioct[kbin][rbin][ioct]);
	  }
	fflush(stdout);
	los_low=rupp[rbin];
	printf("\n");
