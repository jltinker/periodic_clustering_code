...
main(int argc,char **argv)
{
  ...
  if (argc < 16 )  {
    fprintf(stderr,"usage: 2dx-corr rmin rmax nbin rcube xmin xmax vscale ") ;
    fprintf(stderr,
	    "file1 format1 time1 weight1 file2 format2 time2 weight2 [output#] [los axis->0,1,2,3]\n");
    die(" ") ;
  }
  ...
  /*
   * currently only doing the CROSS-correlation function.
   */
  iauto=0 ;
   ...
   */
  sscanf(argv[11],"%f",&weight0) ;
  weight0=1;
  weight1 = (float *) calloc(np1,sizeof(float)) ;	
  for (i=0;i<np1;i++)  {
    weight1[i]=weight0 ;
  }
  sscanf(argv[15],"%f",&weight0) ;
  weight0=1;
  ...
  if(argc>16)
    {
      redshift_flag=atoi(argv[16]);
      if(redshift_flag>0)iredshift=1;
    }
  if(argc>17)
    velocity_axis=atoi(argv[17]);
  else
    velocity_axis=3;
