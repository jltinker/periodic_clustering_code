...
main(int argc,char **argv)
{
  ...
  float rpi,rsigma,             /* coordinates in (pi,sigma) */
    ...
    *xi_qp,*xi_20,**xi_qp_jack,**xi_20_jack,*xi_hexa,**xi_hexa_jack,
  ...
  FILE *jackfile[200];
  char jackname[100];
  float RP_MIN=0.1288,phi_min,phi_range;

  FILE *covarfile;
  char covarname[100],aa[100];
  double **covar_mono,**covar_qp,**covar_20;
  float ***xi_jack;
  int njack=5,njack3,ix,iy,iz,ijack,ijack2;
   ...
   * The range DELTA_PHI will be set at each radial bin.
   */
  nrbin=18;
  //  DELTA_PHI=pi/2.0/nrbin;
  ...
  */
  ...
  for (i=0;i<np1;i++)  {		    
	...
	continue ;
      ...
      // check for fiber-collision distance, rp=0.12
      ...
      if(rsigma<RP_MIN)continue;
      phi=asin(dz/r);

      if(kbin)
	{
	  //	  if(phi>PI/2 - asin(RP_MIN/rupp[kbin-1]))continue;
	  if(asin(rsigma/r)<asin(RP_MIN/rupp[kbin-1]))continue;
	  DELTA_PHI = (PI/2 - asin(RP_MIN/rupp[kbin-1]))/nrbin;
	}
      else
	DELTA_PHI = PI/2/nrbin;
  ...
  for (kbin=0;kbin<nbin;kbin++)  {	/* loop over proj-sep bins */
    ...
    if(rbin){
      phi_range = (PI/2 - asin(RP_MIN/rupp[kbin-1]));
      phi_min = acos(RP_MIN/rupp[kbin-1]);
      DELTA_PHI = (PI/2 - asin(RP_MIN/rupp[kbin-1]))/nrbin;
    } else {
      phi_range = PI/2;
      phi_min = 0;
	...
	vol*=(-sin(phi_low)+sin(phi_low+DELTA_PHI))*2.0;

	//	vol*=(-cos(0.5*pi-phi_low)+cos(0.5*pi-(phi_low+DELTA_PHI)))*2.0;
	 ...
	 */
	//	phi=pi/2 - acos(phibar[kbin][rbin]);
	phi=pi/2 - (phibar[kbin][rbin]);
	/*
	if(kbin)
	  phi=pi/2 - DELTA_PHI*(rbin+0.5) + asin(RP_MIN/rupp[kbin-1]);
	else
	  phi=pi/2 - DELTA_PHI*(rbin+0.5);
	*/
	xi_mono[kbin]+=xi*sin(phi)*DELTA_PHI;
	xi_quad[kbin]+=5*(3*cos(phi)*cos(phi)-1)/2.0*sin(phi)*DELTA_PHI*xi; 
	xi_hexa[kbin]+=9*(35*pow(cos(phi),4.0)-30*cos(phi)*cos(phi)+3)/8.*sin(phi)*DELTA_PHI*xi; 

	for(i=0;i<njack3;++i)
	  {
	    jackweight = weight0 - xi_jack[i][kbin][rbin];
	    xi = jackweight/(weightrandom*(njack3-1.)/njack3) - 1;
	    xi_mono_jack[i][kbin] += xi*sin(phi)*DELTA_PHI;
	    xi_quad_jack[i][kbin] += 5*(3*cos(phi)*cos(phi)-1)/2.0*
	      sin(phi)*DELTA_PHI*xi; 
  ...
  for(i=1;i<nbin;++i) {
    //xi_bar[i] = qtrap(func1,log(0.1),log(rupp[i]))*3/(rupp[i]*rupp[i]*rupp[i] - 0.1*0.1*0.1);
