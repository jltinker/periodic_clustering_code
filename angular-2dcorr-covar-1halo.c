...
main(int argc,char **argv)
{
  ...
  */
   ...
   */
  haloid = malloc(np1*sizeof(int));
  fp2 = fopen("test1.mock_halo","r");
  for(i=0;i<np1;++i) 
    fscanf(fp2,"%d",&haloid[i]);
  fclose(fp2);
  ...
  for (i=0;i<np1;i++)  {		    
	...
	continue ;
      ...
      /* If NOT in same halo, skip.
       */
      if(haloid[j] != haloid[i])
	continue;
