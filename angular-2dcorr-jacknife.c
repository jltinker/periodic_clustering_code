...
main(int argc,char **argv)
{
  ...
  double sigoct[NBINMAX1][RBINMAX1][8],
    pioct[NBINMAX1][RBINMAX1][8];
  ...
  for (kbin=0;kbin<nbin;kbin++)  {	/* loop over proj-sep bins */
    ...
    /* Print out the average radius.
     */
    printf("%e ",rbar[kbin]);
    fflush(stdout);

    /* At each radius, calculate the values for each octant
     */
    for(ioct=0;ioct<8;++ioct) 
      {	
	...
	for (rbin=0;rbin<nrbin;rbin++)
	  {
	    weight0=0;
	    for(i=0;i<7;++i)
	      weight0+=weightsum2d[kbin][rbin][i];
	    ...
	    weightrandom=np1*avgweight1*density*vol*7./8.*7./8. ;	
	    ...
	    xi_mono+=xi*sin(phi)*DELTA_PHI;
	    xi_quad+=(3*cos(phi)*cos(phi)-1)/2.0*sin(phi)*DELTA_PHI*xi; 
	  }	 /* next angular bin */
