...
main(int argc,char **argv)
{
  ...
  float ***xi_jack, q_perp, q_par, rsearch;
  int njack=5,njack3,ix,iy,iz,ijack,ijack2,iout,noutbin,binsum;
  float xi_3wedgeA, xi_3wedgeB, xi_3wedgeC;

  if (argc < 13 )  {
    fprintf(stderr,"usage: 2dcorr rmin rmax nbin rcube xmin xmax vscale ") ;
    fprintf(stderr,
	    "file1 format1 time1 weight1 los_axis->0,1,2,3 njack [MOCK NUMBER] [OUTBIN=5] [qperp=1] [qpar=1]\n");
    exit(0);
  }
  ...
  if(argc>14)
    sprintf(covarname,"xi2d_a0.6542_%s.dat",argv[14]);
  //    sscanf(argv[14],"%s",covarname);
  else
    sprintf(covarname,"xi2d.covar");
  if(argc>15)
    noutbin = atoi(argv[15]);
  else
    noutbin = 1;
  if(argc>16)
    q_perp = atof(argv[16]);
  ...
  */
  ...
  if(velocity_axis<0)
    {
      iredshift = 0;
      redshift_flag = 0;
      velocity_axis = abs(velocity_axis);
    }
  fprintf(stderr,"iredshift=%d velocity_axis=%d\n",iredshift,velocity_axis);
  ...
  if(max(1/q_perp,1/q_par)>1)
    rsearch = rmax*max(1/q_perp, 1/q_par);
  else
    rsearch = rmax;
  fprintf(stderr,"covar3>  rsearch= %f qperp=%f qpar=%f\n",rsearch,q_perp,q_par) ;
  linklist3d(np2,x2,y2,z2,0.0,rcube,rsearch,&listbods,&lattice,&nlattice) ;
  ...
  for (i=0;i<np1;i++)  {		    
    ...
    nrmax=np2 ;
    if (iauto==1)  {	
      rfind2_3d(x2,y2,z2,0.0,rcube,rsearch,listbods,lattice,nlattice,
	     xp,yp,zp,indx,rsqr,&nrmax,i)  ;
    }  else  {
      rfind_3d(x2,y2,z2,0.0,rcube,rsearch,listbods,lattice,nlattice,
	...
	continue ;
      ...
      // rescaling for different cosmologies!
      dz = dz*q_par;
      dx = dx*q_perp;
      dy = dy*q_perp;
  ...
  // if we're box-rescaling, shift the density by the rescaled volume
  density = density/(q_perp*q_perp*q_par);
  fprintf(stderr,"Rescaled volume factor; %f\n",q_perp*q_perp*q_par);
  ...
  for (kbin=0;kbin<nbin;kbin++)  {	/* loop over proj-sep bins */
	...
	// print out the two-d correlation function
	fprintf(fp,"%d %d %e %.0f %f\n",kbin,rbin,xi,weight0,weightrandom);
  ...
  // NOW do the 5-bin fiducial output
  rlow = rmin;
  binsum = noutbin;
  for (kbin=0;kbin<nbin;kbin+=binsum)  {	/* loop over proj-sep bins */
    ...
    xi_3wedgeA = xi_3wedgeB = xi_3wedgeC = 0;
    for (rbin=0;rbin<nrbin;rbin++)
      {
	weight0=0.0 ;
	...
	for(i=0;i<binsum;++i)
	  {
	    for (ioct=0;ioct<8;ioct++)  {		/* add up values from octants */
	      weight0+=weightsum2d[kbin+i][rbin][ioct] ;
	    }	    
	  }
	for(i=0;i<binsum;++i)
	  phi += phibar[kbin+i][rbin];
	phi = phi/weight0;

	/* Volume of wedge of spherical shell 
	 */
	phi_low=rbin*DELTA_PHI;
	vol=2./3.*pi*(rupp[kbin+binsum-1]*rupp[kbin+binsum-1]*rupp[kbin+binsum-1]-rlow*rlow*rlow);
	//	printf("%d %d %f %f\n",rbin,kbin,rupp[kbin],rlow);
	vol*=DELTA_PHI*2.0;
	/*vol*=(cos(phi_low)-cos((phi_low+DELTA_PHI)))*2.0;*/
	weightrandom=np1*avgweight1*density*vol ;	
	...
	if(rbin<nrbin/3)xi_3wedgeA += xi*DELTA_PHI/0.3333;
	if(rbin>=nrbin/3 && rbin<2*nrbin/3)xi_3wedgeB += xi*DELTA_PHI/0.33333;
	if(rbin>=2*nrbin/3)xi_3wedgeC += xi*DELTA_PHI/0.33333;

      }	 /* next angular bin */

    // print out the two-d correlation function
    fprintf(stdout,"%d %e %e %e %e %e %e %e\n",kbin/binsum,xi_mono[kbin],xi_quad[kbin],xi_wedge1[kbin],xi_wedge2[kbin],
	    xi_3wedgeA, xi_3wedgeB, xi_3wedgeC);

    
    rlow=rupp[kbin+binsum-1] ;
