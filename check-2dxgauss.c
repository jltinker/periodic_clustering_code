...
void angular_dependence(float rsig, float rpi, float dvz, float x1, float y1, float z1, 
			...
			float vx2, float vy2, float vz2, int rbin, char *);
...
main(int argc,char **argv)
{
  ...
  if (argc < 13 )  {
    ...
    fprintf(stderr,
	    "file1 file 2 format time1 weight1 (vels y/n) [xtra files]\n");
    die(" ") ;
  }
  ...
  NUMBER_OF_FILES=(argc-14)/2;
  fprintf(stderr,"Number of extra (pairs of) files= %d\n",NUMBER_OF_FILES);

  if((argc-14)%2  ==1)
    {
      sprintf(outfilename,"%s",argv[13+NUMBER_OF_FILES*2+1]);
      fprintf(stderr,"Root of output filename is [%s]\n",argv[13+NUMBER_OF_FILES*2+1]);
    }
  else
    {
  ...
  fp=fopen(argv[8],"r") ;
  sscanf(argv[11],"%f",&time1) ;
  if (fp==NULL)  
    die("couldn't open first input file") ;

  /* read position and velocity data for second object list */
  fp2=fopen(argv[9],"r") ;
  sscanf(argv[11],"%f",&time1) ;
  if (fp==NULL)  
 ...
 LOOP1:
  ...
  if(LOOPCNT>=1)
    {
      fprintf(stderr,"Opening new file1 %d: [%s]\n",LOOPCNT,argv[13+LOOPCNT*2-1]);
      fprintf(stderr,"Opening new file2 %d: [%s]\n",LOOPCNT,argv[13+LOOPCNT*2]);
      fp=fopen(argv[13+LOOPCNT*2-1],"r");
      fp2=fopen(argv[13+LOOPCNT*2],"r");
      free(x1);
      free(y1);
      free(z1);
      free(vx1);
  ...
  /* NB! Currently we only expect that we're using ascii files
   */
  if (strcmp(argv[10],"a")==0)  { 
    /*Don't even ask...*/
    fprintf(stderr,"");
    getposvel_ascii(fp2,&x2,&y2,&z2,&vx2,&vy2,&vz2,&np2) ;
  }
  else {
    die("illegal type for file1") ;
  }
