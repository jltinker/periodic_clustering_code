...
void redshift_distortions(float *z, float *vz, float rcube, int np);
...
main(int argc,char **argv)
{
  ...
  if (argc != 13 )  {
    ...
    fprintf(stderr,
	    "file1 format1 time1 weight1 [vels] [phi_low]\n");
    die(" ") ;
  }
  ...
  if(iredshift)
    {
      redshift_distortions(z1,vz1,rcube,np1);
      if(!iauto)
	redshift_distortions(z2,vz2,rcube,np2);
    }
  ...
  for (i=0;i<np1;i++)  {		    
    ...
    ********************************************/
	...
	rbin=nrbin+1;
      ...
      if(rbin==6 && kbin==0)
	printf("CHECK %f %f %f %f %f\n",r,dz,vz1[i],vz2[j],dz-(vz2[j]-vz1[i])/100);
