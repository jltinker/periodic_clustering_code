...
main(int argc,char **argv)
{
  ...
  for (kbin=0;kbin<nbin;kbin++)  {	/* loop over proj-sep bins */
	...
	if (weight0>0.0)  {
	  ...
	  pibar[kbin][rbin]=zmax/nrbin*(rbin+0.5);
	}
    
	/* compute xi, dividing summed weight by that expected for a random set */
	vol=pi*(rupp[kbin]*rupp[kbin]-rlow*rlow)*(zmax/nrbin)*2.0;

	/* NB above equation assumes that z-bins are linear, not logs. */
	...
	printf("%f %f %f %f %f\n",xi,sigbar[kbin][rbin],pibar[kbin][rbin],rupp[kbin],zmax/nrbin*(rbin+1));
	/*
	printf("%f\n",xi);
