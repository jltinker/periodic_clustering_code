...
main(int argc,char **argv)
{
  ...
  double **covar_matrix,*ndens,total_volume,deltar;
  double **xi_jacks,xi_full[NBINMAX1],x_bar[NBINMAX1],e8[NBINMAX1],xi_avg[NBINMAX1],xnp,**random_pairs;
  ...
  for (i=0;i<np1;i++)  {		    
      ...
      if(ijack>=njack3)
	{
      ...
      for(i1=0;i1<njack3;++i1)
	{
	  
	  if(j%2==0)
	    {
	      if(i1!=ijack)
		xi_jacks[i1][kbin]+=weight;
	    }
	  else
	    {
	      if(i1!=ijack2)
  ...
  /* Get the number density of each jack sample.
   */
  /*
  for(i=0;i<njack3;++i)
    ndens[i]=0;
  ...
  vol = rcube*rcube*rcube*(njack3-1.)/njack3;
  for(i=0;i<njack3;++i)
    ndens[i]=(np2-ndens[i]);
  */

  /* Read in the random pairs.
   */
  /*
  if(!(fp = fopen("random_pairs","r")))
    {
      fprintf(stderr,"ERROR could not open [random_pairs]\n");
      exit(0);
    }
  fscanf(fp,"%d",&k);
  xnp = np1*np2/(k*k*1.0);
  for(i=0;i<nbin;++i)
    for(j=0;j<njack3;++j){
      fscanf(fp,"%d",&k); random_pairs[j][i]=k*xnp; }
  fclose(fp);

  fp = fopen("random_pairs.tmp","w");
  fprintf(fp,"%d\n",np1);
  */
  for (kbin=0;kbin<nbin;kbin++)  {	/* loop over radial bins */
    ...
    */
    ...
    /* get the mean delta within this radius
     */
    x_bar[kbin] = 0;
    for(i=0;i<=kbin;++i)
      {
	if(i>0)
	  deltar = rupp[i]-rupp[i-1];
	else
	  deltar = rupp[i];
	x_bar[kbin] += xi_full[i]*deltar*rbar[i];
	//printf("%d %d %e %e %e\n",kbin,i,x_bar[kbin],deltar,rbar[i]);
      }
    x_bar[kbin] *= 2/rbar[kbin]/rbar[kbin];

    rlow=rupp[kbin] ;

    /* Calculate the xi for the jack samples
     */
    weightrandom*=8./7.;
    total_volume = rcube*rcube*rcube*(njack3-1.)/njack3;
    i=0;
    //    printf("%e %e\n",weight0/weightrandom,xi_jacks[0][kbin]/(2*total_volume*ndens[i]*vol*ndens[i]));


    xi_avg[kbin] = 0;
    for(i=0;i<njack3;++i)
      {
	//	printf("JACK %d %d %.0f %.0f %f\n",kbin,i,weight0 - weightsum[kbin][i],
	//	       xi_jacks[i][kbin]*(njack3)/(njack3-1.0),xi_jacks[i][kbin]/(weight0 - weightsum[kbin][i]));

	// weightrandom = vol/total_volume*ndens[i]*ndens[i]/2;
        //weightrandom = vol/total_volume*pow(np1*(njack3-1.)/njack3,2.0)/2.0;
	//	weightrandom = random_pairs[i][kbin];
	jackweight = xi_jacks[i][kbin];
	// fprintf(fp,"%.0f\n",jackweight);
        //xi_jacks[i][kbin] = (jackweight/(weightrandom) - 1)*2*zmax;
	xi_jacks[i][kbin] = (jackweight/(weightrandom*(njack3-1.)/njack3) - 1)*2*zmax;
	//	printf("%d %d %e %e\n",kbin,i,xi_jacks[i][kbin],rbar[kbin]);
	xi_avg[kbin]+=xi_jacks[i][kbin]/njack3;
      }
  ...
  for(i=0;i<nbin;++i)
    for(j=0;j<nbin;++j)
      for(k=0;k<njack3;++k) { 
	covar_matrix[i][j] += (xi_jacks[k][i] - xi_full[i])*
	  (xi_jacks[k][j] - xi_full[j])*(njack3-1.)/njack3;
      }


  /* get the factor for the projected matter density
   */
  //sscanf(argv[7],"%f",&vscale) ;
  omega_m = atof(argv[16]);
  fprintf(stderr,"Omega_m= %f [%s]\n",omega_m,argv[16]);
  fac = 2.775e11/1.0E12*omega_m;

  for(i=1;i<nbin;++i)
    fprintf(stdout,"%f\t%e\t%e\t%e\n",rbar[i],fac*(x_bar[i]-xi_full[i]),xi_full[i],x_bar[i]);
