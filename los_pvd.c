...
main(int argc,char **argv)
{
  ...
  double rbar[NBINMAX1],	/* mean separation in radial bin */
    ...
    vlos2[NBINMAX1][8],           /* line-of-sight dispersion */
  ...
  /* setting zmax to be 40 Mpc/h to match sloan observations */
  zmax = 40.0;
  ...
  for (i=0;i<np1;i++)  {		    
    ...
    ********************************************/
	...
	dvz=(vz2[j]-vz1[i]);
      ...
      weight=weight1a*weight2a ;
      weightsum[kbin][ioct]+=weight ;
      rbar[kbin]+=weight*r ;
      npair[kbin]++ ;
      vstream[kbin][ioct]+=dvz;
      vlos2[kbin][ioct]+=(vz1[i]-vz2[j])*(vz1[i]-vz2[j]);
    }
  }
...
/* compute output values of velocity statistics */
	...
	ovz=sqrt(vz0*fac-ovstream*ovstream) ;
 ...
 */
	...
	for (ioct=0;ioct<8;ioct++)  {
	    ...
	    diff=sqrt((vz0-vlos2[kbin][ioct])*fac-vstream1*vstream1*0.9999)
                 -ovz ;    /* factor 0.9999 prevents sqrt(negative) error */
	    errvz+=diff*diff ;
	}

	errxi=sqrt(errxi*8./7.) ;
	errvstream=sqrt(errvstream*8./7.) ;
	errvz=sqrt(errvz*8./7.) ;
	
	/**TINKER prevents nan's**/
	if(isnan(ovpar)||isnan(errvpar))
	  ovpar=errvpar=-1;

	printf("%7.4f %7.4f %7.4f %10.3e %10.3e %10.3f %10.3f %13.0f\n",
	       rlow,rupp[kbin],rbar[kbin],ovstream,errvstream,ovz,errvz,weight0);
