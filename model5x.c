...
int main(int argc, char **argv)
{
  float *v,*r,*xi,*sig,**xi2d,rsig,rpi,y,realrad,*y2xi,*y2sig,*y2v,x1,x2,x3,x4,x5,x6,
    ...
    *sigr1,*sigr2,*sigt1,*sigt2,*mu1,*mu2,*y2sigr1,*y2sigr2,*y2sigt1,*y2sigt2,*y2mu1,*y2mu2,
    sr1,sr2,st1,st2,m1,m2,*fact,*facr,err,xup,yup,halomass1,halomass2,sigma_8,omega_m,chi2;
  int i=0,j,k,npts,ncol,Ny,i1,i2,gridsize;
  FILE *fp,*fp2;

  halomass1=atof(argv[3]);
  halomass2=atof(argv[4]);
  OMEGA_M=omega_m=atof(argv[5]);
  SIGMA_8=sigma_8=atof(argv[6]);
  ...
  if(argc<6)
    kill("model5x xifile xi2dfile log_2(m1) log_2(m2) omega_m sigma_8 > out");
	...
	for(xi2d[i][j]=0,k=-Ny+1;k<=Ny;++k)
	  {
	    ...
	    splint(r,xi,y2xi,npts,realrad,&x3);

	    x1=sigma1rad(realrad,halomass1,omega_m,sigma_8);
	    x2=sigma1rad(realrad,halomass2,omega_m,sigma_8);
	    sr1=sqrt((x1*x1+x2*x2)/2);
	    x1=sigma2rad(realrad,halomass1,omega_m,sigma_8);
	    x2=sigma2rad(realrad,halomass2,omega_m,sigma_8);
	    sr2=sqrt((x1*x1+x2*x2)/2);
	    x1=sigma1tan(realrad,halomass1,omega_m,sigma_8);
	    x2=sigma1tan(realrad,halomass2,omega_m,sigma_8);
	    st1=sqrt((x1*x1+x2*x2)/2);
	    x1=sigma2tan(realrad,halomass1,omega_m,sigma_8);
	    x2=sigma2tan(realrad,halomass2,omega_m,sigma_8);
	    st2=sqrt((x1*x1+x2*x2)/2);
	    /*
	    sr1=0.5*(sigma1rad(realrad,halomass1,omega_m,sigma_8)+
		     sigma1rad(realrad,halomass2,omega_m,sigma_8));
	    sr2=0.5*(sigma2rad(realrad,halomass1,omega_m,sigma_8)+
		     sigma2rad(realrad,halomass2,omega_m,sigma_8));
	    st1=0.5*(sigma1tan(realrad,halomass1,omega_m,sigma_8)+
		    sigma1tan(realrad,halomass2,omega_m,sigma_8));
	    st2=0.5*(sigma2tan(realrad,halomass1,omega_m,sigma_8)+
		    sigma2tan(realrad,halomass2,omega_m,sigma_8));
	    */
	    m1=0.5*(mu1rad(realrad,halomass1,omega_m,sigma_8)+
		    mu1rad(realrad,halomass2,omega_m,sigma_8));
	    m2=0.5*(mu2rad(realrad,halomass1,omega_m,sigma_8)+
		    mu2rad(realrad,halomass2,omega_m,sigma_8));
	    
	    x2=integrate_v_los_prob(dv,sr1,sr2,st1,st2,m1,m2,0.6,0.6,rsig,y);
	    /*fprintf(stderr,"%f %f %f %f %f %f %f\n",m1,m2,sr1,sr2,st1,st2,x2);*/
			   ...
			   float mu1, float mu2, float ft, float fr, float rs, float rp)
{
  ...
  /*
  s1=qromo(func2,-6000,6000,midpnt); 
  return(s1/g_costheta);
  */

  /* Let's try a different integration routine
   */
  prev=trapzd(func2,-2000,2000,n);
  while(error>tol) {
    n++;
    s1=trapzd(func2,-2000,2000,n);
