...
/** Now we are adding, to model5.c, the chi2 minimization in order to
    determine the cosmological parameters from the given data.
    ----------------------------------------------------------------------- 
    This is a copy of model4.c, but now we are inserting the numerical fits
    for the velocities (for the 2-gaussian distributions) into the code
    hard wired. First, we'll just have omega_m and sigma_8 be input
...
/** THE BIG ROUTINE **/
float calc_xi2d(char **argv, float omega_m, float sigma_8);

/** GLOBALS FOR USE WITH QROMO (or whatever integration routine) **/
float g_logr,
  g_sintheta,
  g_costheta,
  g_vlos1,
...
{
  int i,j,n2=50;
  float **chi2,omega_m,sigma_8;

  if(argc<4)
    kill("model4 xifile xi2dfile log_2(m) [sigma_8] > out");

  if(argc>4)
    {
      SIGMA_8=sigma_8=atof(argv[4]);
      for(i=1;i<=n2;++i)
	{
	  OMEGA_M=omega_m=(i-0.5)/n2;
	  printf("%f %f %f\n",omega_m,sigma_8,calc_xi2d(argv,omega_m,sigma_8));
	  fflush(stdout);
	}
    }
  else
    {
      chi2=matrix(1,n2,1,n2);
      for(i=1;i<=n2;++i)
	for(j=1;j<=n2;++j)
	  {
	    OMEGA_M=omega_m=(i-0.5)/n2;
	    SIGMA_8=sigma_8=(j-0.5)/n2+0.3;
	    chi2[i][j]=calc_xi2d(argv,omega_m,sigma_8);
	    printf("%f %f %f\n",omega_m,sigma_8,chi2[i][j]);
	    fflush(stdout);
	  }
    }
...
float calc_xi2d(char **argv, float omega_m, float sigma_8)
{
  float *v,*r,*xi,*sig,**xi2d,rsig,rpi,y,realrad,*y2xi,*y2sig,*y2v,x1,x2,x3,x4,x5,x6,
    ...
    *sigr1,*sigr2,*sigt1,*sigt2,*mu1,*mu2,*y2sigr1,*y2sigr2,*y2sigt1,*y2sigt2,*y2mu1,*y2mu2,
    sr1,sr2,st1,st2,m1,m2,*fact,*facr,err,xup,yup,halomass,chi2;
  ...
  for(j=1;j<i;++j)
    {
      ...
      chi2+=(x1-x2)*(x1-x2)/err/err;
    }
  fclose(fp);
  free_vector(r,1,NMAX);
  free_vector(xi,1,NMAX);
  free_vector(y2xi,1,NMAX);
  free_matrix(xi2d,1,N2d,1,N2d);
  return(chi2);
}
