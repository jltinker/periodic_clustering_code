...
main(int argc,char **argv)
{
  ...
  int njack=5,njack3,ix,iy,iz,ijack,ijack2,i1,COVAR2D = 0, MAKE_RANDOMS = 0;
  float xk;
  ...
  if (strcmp(argv[9],"a")==0)  { 
    /*Don't even ask...*/
    fprintf(stderr,"");
    if(MAKE_RANDOMS) { np1 = 1e5;
      getposvel_random(rcube,&x1,&y1,&z1,&vx1,&vy1,&vz1,&np1,324908) ; }
    else
  ...
  */
  ...
  if(!MAKE_RANDOMS)
      ...
      if(!(fp = fopen("random_pairs","r")))
	{
      ...
      fscanf(fp,"%d",&k);
      xnp = np1/(k*1.0)*np1/(k*1.0);
      printf("%f\n",xnp);
      for(i=0;i<nbin;++i)
	for(j=0;j<njack3;++j){
	  fscanf(fp,"%f",&xk); random_pairs[j][i]=xk*xnp; }
  ...
  for(i=1;i<nbin;++i)
    fprintf(stdout,"%f\t%e\t%e\t%e\t%e\n",rbar[i],xi_full[i],sqrt(covar_matrix[i][i]),e8[i],xi_avg[i]);

  fp = fopen(covarname,"w");
  for(i=1;i<nbin;++i)
    for(j=1;j<nbin;++j)
      fprintf(fp,"A %d %d %e\n",i,j,covar_matrix[i][j]);
  fclose(fp);
}
